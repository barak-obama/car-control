function scorchReducedGamePermanentConditions
...
load('PermanentConditionsForGame')
...
tanks{1}.x = Parameters.x; tanks{1}.y = Parameters.y;  tanks{2}.x = Parameters.xEnemy;  tanks{2}.y = Parameters.yEnemy;
params.tanksize = Parameters.tanksize; params.canonsize = Parameters.canonsize;status.wind = Parameters.wind;
params.gravity = Parameters.gravity;
params.simulationdelta = Parameters.simulationdelta;params.tmax = Parameters.tmax;terrain = Parameters.terrain;
params.missleblastsize = Parameters.blastrad;
...
while (length(tanks)>1)
  ...
  if (tanks{status.currtank}.ishuman)
    ...
    % human player
    set(status.hfig,'KeyPressFcn',@keypress);
    uiwait(status.hfig);
